...
export default function Chat({setpage,state}){
    ...
    return (
        ...
        <div style={{height:"100vh", display:"flex",background:state? "radial-gradient(circle,#eeaeca 0%, #1D1845 100%)":"radial-gradient(circle,#eeaeca 0%, #94bbe9 100%)",backgroundSize:"cover" , flexDirection:'column',justifyContent:"space-between",fontFamily:'caveatfont'}}>
            <div style={{display:"flex",justifyContent:"space-between",  boxShadow: "rgb(228, 229, 230) -5px 2px 5px 0px",
        ...
        paddingLeft: "22px",
         transition:"0.3s"}}>
            <h1 >
      Books chat<span style={{fontSize:"19px",marginLeft:"3px" ,marginTop:"7px",display:state?"none":"inline"}}> 👥</span>
    </h1>
    <button  className="chatbutton"  onClick={go_to_home} style={{boxShadow: "-2px -1px 6px 0px",marginRight:"22px",backgroundColor:"#eeaeca",height:"30px",border:"none",borderRadius:"25px",padding:"12px",display:"flex",justifyContent:"center" ,alignItems:"center",fontSize:"18px"}}>Back to Home</button>
    </div>
            ...
            <input name="message" placeholder=' ⌨️ Write the message here'  value={message} onChange={message_Change}style={{height:"40px",width:"95%",backgroundColor: "#e2dfe4",fontSize:'21px',color:state ? "#e2dfe4" :"#1E1B4B"}} />
            <button className="iconsend" style={{border:"none",background:"none",padding:"0",color:state ? "#e2dfe4" :"#1E1B4B"}}><FontAwesomeIcon   icon={faPaperPlane} style={{fontSize:"1.5rem"}}/></button>
            </form>
        </div>
    )
}
